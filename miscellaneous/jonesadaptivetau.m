...
while t < tfinal
    ...
    fprintf('t = %.4e | tau = %.4e | Type: %s | X = [%d, %d, %d, %d, %d]\n', ...
        t, tau, ...
        string(type), X(1), X(2), X(3), X(4), X(5));  % Add T* and V
...
% Replace existing plot commands with:
...
% NA
rev = []; % Indices of reversible reaction pairs (1-based indexing)
...
hor = [2;  % A: involved in 2nd-order reactions (v1, v3)
       1;  % CC: 1st-order consumption (v7)
       2;  % T: 2nd-order reactions (v1, v3, v4, v9)
       1;  % T*: 1st-order reactions (v6, v10)
       2]; % V: 2nd-order reactions (v4, v9)
% Corresponding stoichiometric coefficients
nuHor = [1;  % A: max consumption = 1 (v1, v3)
         1;  % CC: consumption = 1 (v7)
         1;  % T: max consumption = 1 (v1, v3, v4, v9)
         1;  % T*: max consumption = 1 (v6, v10)
         1]; % V: max consumption = 1 (v4, v9)
...
while count < steps && t < tfinal
    ...
    if reactionIndex > 0
        % Fire the reaction with non-negative enforcement
        X = X + V(:, reactionIndex);
        X = max(X, 0);  % CRITICAL: Prevent negative populations
        t = t + dt;
    else
        t = inf;
        break;
    ...
    % DEBUG: Print state for Jones model (5 species)
    if mod(count, 10) == 0  % Print every 10 steps
        fprintf('t=%.4f: A=%d, CC=%d, T=%d, T*=%d, V=%d\n', ...
...
% X = [A; CC; T; T*; V]
a = [
    c(1)*X(1)*X(3);                     % v1: γ·A·T
    c(7)*c(11)*X(4);                     % v10: δ·NN·T*
    c(8)*c(12)*X(2);                     % v11: μ·NC·CC
    c(2);                                % v2: λ
    c(3)*X(1)*X(3)/(c(4) + X(1));        % v3: a·A·T/(K+A)
    c(5)*X(3)*X(5);                      % v4: k·T·V
    c(6)*X(3);                           % v5: d·T
    c(7)*X(4);                           % v6: δ·T*
    c(8)*X(2);                           % v7: μ·CC
    c(10)*X(5);                          % v8: c·V
    c(9)*c(5)*X(3)*X(5)                  % v9: α·k·T·V
    ];
